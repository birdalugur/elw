function [fh,xh,gh,H,itct,fcount,retcodeh] = csminwel_ks(fcn,x0,H0,grad,crit,nit,varargin)
%[fhat,xhat,ghat,Hhat,itct,fcount,retcodehat] = csminwel(fcn,x0,H0,grad,crit,nit,varargin)
%
%       Modified by Katsumi Shimotsu to suppress the screen output, g1.mat and H.mat
%
...
%   disp(sprintf('f at the beginning of new iteration, %20.10f',f))
 ...
 %     disp([sprintf('x = ') sprintf('%15.8g %15.8g %15.8g %15.8g\n',x)]);
   %-------------------------
   itct=itct+1;
   [f1 x1 fc retcode1] = csminit_ks(fcn,x,f,g,badg,H,varargin{:});
   %ARGLIST
...
% original         disp('Cliff.  Perturbing search direction.')
         [f2 x2 fc retcode2] = csminit_ks(fcn,x,f,g,badg,Hcliff,varargin{:});
         %ARGLIST
         ...
         if  f2 < f
            if retcode2==2 | retcode2==4
               ...
               % g2
% original               badg2
% original               save g2 g2 x2 f2 varargin
               %ARGLIST
               %save g2 g2 x2 f2 P1 P2 P3 P4 P5 P6 P7 P8 P9 P10 P11 P12 P13;
            end
            if wall2
% original               disp('Cliff again.  Try traversing')
               if norm(x2-x1) < 1e-13
                  f3=f; x3=x; badg3=1;retcode3=101;
               else
                  gcliff=((f2-f1)/((norm(x2-x1))^2))*(x2-x1);
                  if(size(x0,2)>1), gcliff=gcliff', end
                  [f3 x3 fc retcode3] = csminit_ks(fcn,x,f,gcliff,0,eye(nx),varargin{:});
                  %ARGLIST
                  ...
                  if retcode3==2 | retcode3==4
                     ...
                     % g3
% original                     badg3
% original           save g3 g3 x3 f3 varargin;
                     %ARGLIST
